...
SET PATH=%CURRENT_DIR%msys_trimmed/usr/bin;%PATH%
...
SET SILENT=1
SET MAKE_ISO_XORRISO=%CURRENT_DIR%msys_trimmed/usr/bin/xorrisofs
...
IF NOT EXIST %CURRENT_DIR%libyaul-examples/ (
  ...
  FOR /d %%i IN (*) DO (
    xcopy /s/e/y "%CURRENT_DIR%vscode_template" "%%i"
  )

  @REM Extract latest libyaul gcc and copy libwinpthread-1.dll so that intellisense works on vscode
  unzip %CURRENT_DIR%sh2eb-elf.zip
  cp %CURRENT_DIR%msys_trimmed/usr/bin/libwinpthread-1.dll sh2eb-elf/bin/
  ...
  xcopy /y "%CURRENT_DIR%yaul_patch\strstr.c" "libyaul\lib\string\strstr.c"
  xcopy /y "%CURRENT_DIR%yaul_patch\tlsf.c" "libyaul\kernel\mm\tlsf.c"
